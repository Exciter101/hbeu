    ...
    partial class Coroutines
    {
        ...
        private async static Task<bool> CheckFishLine()
        {
            ...
            if (Me.IsCasting)
            {
                ...
                if (AutoAnglerSettings.Instance.KaarpHuyarp)
                {
                    WoWItem Kaarp = Utility.GetItemInBag(116158);
                    if (Kaarp != null && Kaarp.Use())
                    {
                        AutoAnglerBot.Log("Using Karp");
                        WoWPetControl.SetStance(PetStance.Passive);
                        await Coroutine.Sleep(AutoAnglerSettings.Instance.KaarpDelay * 1000);
                        return true;
                    }

                    WoWItem Huyarp = Utility.GetItemInBag(112633);
                    if (Huyarp != null && Huyarp.Use())
                    {
                        AutoAnglerBot.Log("Using Huyarp");
                        WoWPetControl.SetStance(PetStance.Passive);
