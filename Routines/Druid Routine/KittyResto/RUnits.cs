...
namespace RestoDruid
{
    public class units
    {
        ...
        #region is in range
        public static bool IsInRange(int distance, WoWUnit unit)
        {
            return unit != null && ValidUnit(unit) && unit.Distance <= distance;
        }
        #endregion
        ...
        public static bool MyBuffExists(string Buff, WoWUnit onTarget)
        {
            if (onTarget != null)
            {
                WoWAura aura = onTarget.GetAllAuras().FirstOrDefault(a => a.Name == Buff && a.CreatorGuid == Me.Guid);
                if (aura != null)
                {
                    return true;
                }
        ...
        #region dispel
        public static bool HasDebuff(WoWUnit onTarget)
        {
            if (onTarget != null)
            {
                WoWAura aura = onTarget.GetAllAuras().Where(a => a.IsHarmful).FirstOrDefault();
                if (aura != null)
                {
                    return true;
                }
